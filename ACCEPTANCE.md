...
The [P2 continuation decision](pcb/ELECTRICAL.md#8-continuation-decision-2026-09-07) and [conditional design bounds](pcb/DESIGN_BOUNDS.md) are analysis inputs, not pre-approved test bands. **35-37 V at the TEST feeds is proposed, not hardware-enforced at either limit**; the floor includes hub/interface losses and the ceiling normal ripple. Do not discard the higher accessible-adjustment screen or use 37 V as an actual protection threshold until a real interface enforces and verifies it. The spare PSU remains disconnected.
...
**Incoming fit evidence precedes assembly.** [ASSEMBLY's adopted hole/pin limits](pcb/ASSEMBLY.md#hole-fit-evidence) and [DFM evidence](pcb/DFM_EVIDENCE.md#independent-acceptance) distinguish published maxima from controlled **E procurement envelopes**. Original mechanical figures and all maximum-body boxes are now resolved or explicitly controlled, not unreadable. Independently inspect **every allocated controlled-envelope part, including attrition**, with uncertainty. Verify the separate **0.050 mm radial per-pin pattern limit after common rigid alignment through the insertion length**; together with **+/-0.05 mm hole position on each axis**, it allocates **0.241421 mm diametrally in addition to the 0.10 mm assembly allowance** for the enlarged holes. D1/D2 retain diameter-only 1.10 mm holes and require separate forming/alignment to the actual board pattern. No allocated lot, forming process or insertion yield is approved by the calculations.

Perform and record these checks on **each** unpowered prototype before stress testing:

1. Match markings, references, MPNs, values, population and orientation to the accepted BOM/drawing. Confirm onsemi identity for baseline 1N4007G parts. The current population is unchanged at 14; do not treat an unimplemented P2 candidate as a required fitted part. Any later circuit additions need their own approved BOM/revision. No assumed approval of alternative indicators or optional upgrades.
...
4. Compare the adopted [component-hole schedule](ORDERING.md#3-hole-via-and-solder-process), pin/body/pattern limits and incoming reports with actual finished holes, annular rings and courtyards. Connector, resistor and GDT holes are now selected in the source, not future sizing candidates; D1/D2 remain **1.10 mm finished hole / 2.20 mm pad**. Require actual free insertion, accepted seating/forming and solder fill; enlargement alone is not lot-fit approval. Do not ream a plated hole, force a component or shrink a courtyard to hide interference. Resolve defects through the assembler/design owner.
...
6. Inspect **GDT_AC's whole raised body/electrode/lead span**, accepting only when its measured minimum gap minus uncertainty is **>=2.00 mm above the highest finished PCB top surface** before gel. The drawing targets **body underside 2.50 +/-0.25 mm and complete assembled height <=8.50 mm**, not just body-centre height; these are not actual forming-process approval. Check earth GDTs at **9.00 mm centre pitch** against the controlled **8.21 mm body envelope and 0.10 mm per-side projected pose allowance**: the budgeted body gap is **0.59 mm**, not a 1.00 mm air-gap or impulse-insulation claim. Neither gel nor solder mask excuses contact with the crossing conductor.
...
8. Match processed mask/stencil data and actual SMT joints to the adopted **5.50 x 1.20 mm Ruilon lands at local Y=+/-2.00, X=119.50 mm**. Metric **4.00 mm centre spacing** is selected; the contradictory **0.165-inch** entry and actual solder/profile acceptance remain **W1**, not resolved by the drawing choice. Nominal drill-circle/annulus-to-SMT-aperture distances are **1.239713 / 0.839713 mm** before actual registration/solder allowances. Inspect all four SMT apertures and preserve the below-tube insulation space; a raw F.Paste export is not processed-stencil approval.
...
For the proposed simple negative-voltage clamps, test only the **selected, stated protection scope** on the allocated instrumented unit with correctly oriented external LEDs, a reviewed current-limited fixture and stop limits. Include normal operation, relevant negative connector voltage, startup/turn-off and agreed transients. The former complete eight-case source/LED-reversal immunity matrix is not a required design exit. Change wiring only while isolated; do not prescribe darkness as proof of protection. C4 remains held because no clamp is fitted and ordinary transient behavior is not established.

Measure **at the physical LED**, accounting for probe loading, and capture reverse voltage, forward current, protection current/power and temperature. Respect the **5 V reverse / 20 mA forward** ratings in normal operation and establish acceptable response to the defined pulse exposure; absent pulse ratings are not proof that every brief excursion causes failure. Do not invent blanket surge immunity or silently waive routine/cattle-fence damage. Stop at the pre-approved thresholds and retain failures. Any topology change must pass the full-hub observability tests.
...
A 2 W replacement at the same resistance still dissipates approximately the
same watts; compare its actual mounting/derating and the measured body/lead or
chip/pad temperatures. The candidates in ELECTRICAL section 9 are not fitted
parts or a thermal PASS. Qualifying the real normal supply range is an
alternative to adding circuitry solely to enforce the proposed 37 V ceiling.

At the proposed **37 V**, the zero-drop/Rmin calculation gives **0.631876 W per resistor** and a **71.534 degrees C rating-only local ambient ceiling** in standard mode. Neither number is a measured temperature, an approved stop limit or safe continuous qualification; actual film/body/interface temperatures, source enforcement and all material limits still need evidence. The **40.39597 V accessible-adjustment screen remains relevant while the proposed feed ceiling is unenforced**. See [DESIGN_BOUNDS](pcb/DESIGN_BOUNDS.md#thermal-requirements), not a rounded 0.5 W or 1 W headline rating.
...
The continuation's **4.6681 mA nonuniform far-end corner at 35 V** uses one high-drop/Rmax branch with other branches at zero drop/Rmin. It is a named conditional case, **not an exhaustive minimum or a guaranteed 6 mA floor**. Uniform-load results cannot set the final acceptance current, and new protection loading must be included; see [the current comparisons](pcb/DESIGN_BOUNDS.md#normal-current).
