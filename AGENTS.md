...
* **Wire B LED Return Center-Spine Routing**: To eliminate any potential short circuits with the line-to-earth GDT inputs on Wire A and Wire C at X=131.00mm, the Wire B LED return traces on `B.Cu` are routed down the center spine between the line-to-earth GDT bodies at X=138.62mm. This achieves a generous >5.3mm clearance to the GDT line inputs (Wires A and C) on the left, and >5.3mm clearance to the Earth bus pads on the right, with >1.55mm clearance to all other nets.
