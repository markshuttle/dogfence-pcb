...
The [P2 continuation](pcb/ELECTRICAL.md#8-continuation-decision-2026-09-07) and [conditional design bounds](pcb/DESIGN_BOUNDS.md) propose **35-37 V at the TEST feeds, nominal 36 V**, including normal ripple and hub/interface losses. **Neither limit is hardware-enforced.** This is not an installed-setting approval or permission to ignore accessible adjustment/start-up; the existing higher-voltage screen remains relevant until an accepted interface enforces the window. PSU OVP is not a 37 V limiter.
...
First dry-fit a complete prototype. Do not drill or prepare all lids until that fit and the process have been accepted. [pcb/ASSEMBLY.md](pcb/ASSEMBLY.md) controls the adopted P3 hole, pin-pattern, body and forming limits; [pcb/DFM_EVIDENCE.md](pcb/DFM_EVIDENCE.md) records the visually read figures and calculations. These choices leave the **14-part circuit/BOM unchanged**, with no new C4/C5 protection parts. Controlled envelopes are not passed lot-fit or solder-process evidence.
...
7. Connect each pigtail between its own WAGO and the matching `J_IN` **A, B or C** marking. Connect each actual APEM anode to its channel's `+` and cathode to `-`; verify the supplied lead identification rather than relying solely on assumed red/black colours. **The existing series diode does not guarantee LED survival if an LED or the supply is reversed.** The user accepts indicator damage from accidental low-voltage installation polarity errors and requires installation spares, with quantity still to allocate. Check before power; correct suspect wiring and replace damaged indicators while isolated, not by leaving a dark/miswired branch powered. This does not waive safe wiring or continuous TEST thermal requirements.
...
The continuation's **4.6681 mA nonuniform far-end corner at 35 V** is a conditional example, not an exhaustive minimum or a guaranteed 6 mA floor. A separate zero-drop/Rmin screen at the proposed **37 V** gives **0.631876 W per resistor** and a **71.534 degrees C rating-only local ambient ceiling**. These are not measured temperatures, accepted operating limits or safe continuous qualification. Source enforcement, actual thermal interfaces and 5 m daylight results remain required.
