...
[ELECTRICAL](pcb/ELECTRICAL.md) records the remaining circuit limits: negative clamps are not qualified pulse protection, powered-GDT recovery is unresolved and actual OneGel thermal testing is unperformed. Use [ACCEPTANCE](ACCEPTANCE.md) for controlled prototype qualification and commissioning records, and [RISKS](RISKS.md) for limitations. Do not roll out this draft or rely on it for containment or lightning protection.

[MATERIALS](MATERIALS.md) owns purchases, receipts and allocations, including [resistor history](MATERIALS.md#resistor-procurement-and-history); [ENVIRONMENT_EVIDENCE](pcb/ENVIRONMENT_EVIDENCE.md) owns manufacturer/site evidence and unresolved discrepancies.
...
- Preserve the WAGO through-splice/PCB-tap arrangement. Use three WAGO 221-613 splices and four supports per station. Reconcile total requirements and prototype/spare allowances against the [recorded inventory](MATERIALS.md#1-station-inventory); required quantities are not evidence of purchases.
...
**No energized cattle fencing is permitted near the whole 4 km boundary cable, any station or the hub**, as explicitly confirmed by the user. The former **0.5 m / up to 300 m parallel scenario is withdrawn and historical**, not an accepted installation or current energizer-on/off test requirement. Inspect the whole route, including every station and the hub, before use and maintain the prohibition. If land use changes or the hazard is reintroduced, reassess before use; no numerical safe separation, zero induction or zero lightning exposure is established. Ordinary RF, switching and nearby-lightning exposure remain relevant.
...
| Boundary cable | User-confirmed **AMC Oceanflex CM03/05.100** reels labelled `3 CORE (3 x 35/0.30) TINNED BLACK 100 MTR`. AMC **2026 Update V.3** lists **3 x 2.5 mm^2, Class 5 tinned copper, 7.4 mm maximum OD, 60 V max DC, -30 to +70 degrees C** complete-cable service. Product identity is settled; confirm supplied manufacturing-revision/lot applicability and reconcile the [conditional resistance/strand-construction evidence](pcb/DESIGN_BOUNDS.md#evidence-and-inputs). A model ceiling is not a reel measurement. |
| Core identification | Confirmed colours are **red, black and PLAIN green**, not green/yellow. Plain green is an identified active core, **not PE**. Identify every core end-to-end and record/label its A/B/C assignment at every termination; no colour-to-net mapping is assigned here. **GREEN/YELLOW MUST NOT BE USED AS AN ACTIVE A, B, OR C CONDUCTOR**, including by re-sleeving it. |
...
**No permanent End A/C strap, opposite-end B return or dual-B negative return is permitted.** An End A/C strap can backfeed a broken core; an opposite-end-only B return leaves both sides dark after a complete cut; connecting both B ends to negative masks a B-only break.
...
2. Use **one MEAN WELL LRS-75-36, 36 V / 2.1 A / 75.6 W, for TEST; keep the second disconnected as a spare**. No parallel, series, separate-channel or opposite-end feeding is authorized. Verify receipt/nameplates and the intended unit's output setting against the [hub inventory](MATERIALS.md#3-shed-hub-inventory); an order is not inspected hardware. LRS-75-36 has adjustable output and hiccup overload protection. Set and record only the voltage range authorized by the final electrical/thermal analysis; do not raise it to compensate for poor visibility.
...
The [source-envelope study](pcb/DESIGN_BOUNDS.md#evidence-and-inputs) uses conditional calculations, not hardware-enforced or approved installed voltage limits. Establish actual adjustment, tolerance, ripple and startup behavior, including hub/interface losses in the feed limits; PSU OVP is not a precision operating limiter. Qualifying the real normal source range is an alternative to imposing a new precision cutoff; it does not settle source-malfunction or GDT recovery safety.
...
After receiving the prototypes, first dry-fit a complete assembly. Do not drill or prepare all lids until that fit and the process have been accepted. Use the reviewed [PCB BOM](pcb/BOM.csv), [placement and assembly controls](pcb/ASSEMBLY.md#placement-and-polarity) and [DFM evidence](pcb/DFM_EVIDENCE.md) for received-part identity, lands, retained holes, pin-pattern, body and earth-GDT-forming limits.

The prototype uses 2512 SMT resistors, SMA diodes and **three SMT inter-core GDTs, including GDT_AC**. There are no resistor/diode/AC insertion holes or axial-resistor/AC standoff/forming requirements; the three earth GDTs remain axial. The retained **project-alternative resistor lands differ from TE's recommendation** and require supplier placement/stencil/solder acceptance; body fit is not an exact land-pattern match. Yageo needs [separate review and controlled source updates](ORDERING.md#7-prototype-acceptance-and-later-orders): neither its order nor TE testing authorizes substitution. Controlled envelopes are not accepted lots or assembled hardware.

1. Match each received board to the reviewed revision, BOM and assembly drawing. Inspect workmanship and run the all-five basic checks in [ACCEPTANCE.md](ACCEPTANCE.md). Board outline is nominally 65 x 56 mm with four 3.2 mm mounting holes at the [current controlled positions](pcb/ASSEMBLY.md#physical-guardrails); actual thickness and hole tolerances must suit the supports. No forcing leads into undersized holes, reaming finished plated holes, or field PCB soldering is allowed.
...
5. Trial-fit **all** contents: three WAGO 221-613s, actual incoming/outgoing cable and glands, three pigtails, earth conductors where applicable, supports, both LED bodies, lid seals, plugs, and a suitable terminal screwdriver. Allow cable bend radii, wire strain relief and lid service slack. Close the lid fully and verify no wire or LED body presses on a GDT, solder joint or PCB. Use the [current body/seating and crossing controls](pcb/ASSEMBLY.md#placement-and-polarity), including actual SMT solder seating height; **GDT_AC has no raised-overpass lead-forming or height-gauge requirement**. Inspect the three axial earth GDTs at the unchanged **9.00 mm centre pitch** against their accepted forming drawing and controlled maximum-body/pose envelopes, not an assumed 1 mm air gap. Do not apply retired PR02 standoff/forming instructions to SMT resistors. Record actual clearances and inspect seating, leads and joints again after lid closure/re-entry. Solder mask, tape or gel cannot excuse contact, bad forming or unqualified insulation; the rear B crossing requires the separate workmanship and [qualified insulation/transient checks](ACCEPTANCE.md#qualified-fault-tests).
...
Use [J_EARTH's edge-clearance and inspection requirements](pcb/ASSEMBLY.md#earth-terminal-edge-clearance) in the dry-fit: its unchanged body/pose envelope and full courtyard are **nominally inside the extended east edge**, not supplier-tolerance or measured-fit approval. Inspect actual lot, routed edge, supports/enclosure and wire/tool access; record clearances with uncertainty. Stop for interference or missing fit/access acceptance before potting or preparing the remaining lids.
...
1. Secure animals and isolate **all six boundary ends from the hub, transmitter, PSU and every backup source** as described in [Safety First](#safety-first). Both physical cable ends are at the shed. Verify de-energization before selecting resistance mode. Do not use resistance mode on RUN or TEST wiring.
...
Use the [TE-prototype / BYG23T bounds](pcb/DESIGN_BOUNDS.md), not retired-part values or unreviewed Yageo extrapolations. Their temperature/reference inputs are engineering assumptions, not verified TE test conditions or measured operating temperatures. Screens are not approved source limits, exhaustive minimum current or guaranteed brightness. Catalogue resistor power/derating transfer to this mounting remains unverified, not a demonstrated failure; SMT does not eliminate loss or prove cooler operation. Measure the actual chip/pad/PCB and gel/cable interfaces and qualify 5 m daylight indications for the intended population. No OneGel thermal test has been performed.
