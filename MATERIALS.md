...
The field baseline is **41 stations at 0, 100, ..., 4000 m**, including two separate shed-end boards, with **36 standard + 5 earth-connected stations** and one additional shed control hub. This ledger covers PCB procurement and external system materials; [pcb/BOM.csv](pcb/BOM.csv) controls only PCB population and sourcing metadata, not all system purchases.

**Gate P covers file-verified prototype artifacts, not upload/order authorization or allocation.** The five planned prototypes still need supplier acceptance under [ORDERING](ORDERING.md); production/field [release gates](REMEDIATION.md#release-gates) remain held. [ACCEPTANCE](ACCEPTANCE.md) controls qualification and sample use, not purchase status.
...
**Active prototype BOM only:** **TE Connectivity 35212K2FT / [C4129105](https://jlcpcb.com/partdetail/C4129105)**, 2512 SMT, 2.2 kohm / 2 W / 1% / +/-100 ppm/C, AEC-Q200 qualified. In stock at JLCPCB as an Extended Part. Selected for the five prototype boards (ten fitted resistors plus supplier-defined attrition), not a recorded TE purchase or PCBA-job allocation.

**User-confirmed production order:** **Yageo SR2512FK-7W2K2L / [C876850](https://jlcpcb.com/partdetail/C876850)**. The JLCPCB listing confirms YAGEO, 2512 SMT, 2.2 kohm / 2 W / 1% / 100 ppm/C identity. The user confirms this order with an advised **20-day wait**, not receipt, inspection or PCBA-job allocation; its quantity is not established here. Do not transfer the cancelled PS12 order's 110-piece quantity to Yageo. No Yageo manufacturer mounting, thermal or pulse qualification is claimed from the identity listing.

**Corrected code history:** the earlier **C19451159** pairing with SR2512FK-7W2K2L was a documentation error, not the user's ordered part. The confirmed production order is C876850 above; the rejected pairing is not a separate purchase or a substitute.

**Historical order record, 2026-09-08:** **110 Uni-Royal PS122WF2201T4E / [C2793873](https://jlcpcb.com/partdetail/C2793873), ORDERED into the user's JLCPCB parts library**. The former shared prototype/production plan budgeted 55 two-resistor populations, not approved boards: 10 prototype plus 82 field resistors would have left 18 before attrition if prototypes were not reused. This preserves the original order and planning arithmetic, not available stock.

**Current PS12 disposition, 2026-09-11: CANCELLED AND REFUNDED** by JLCPCB due to distributor stockout. No receipt, inspection or allocation is established by the historical order. Its former 18-part arithmetic balance is not current spare stock and is not transferred to TE or Yageo.

Yageo remains a **production selection awaiting separate engineering review and integration**, not a second implemented BOM. Before production release, review its own manufacturer/rating, land-pattern, thermal, pulse and process evidence; update the controlled BOM/CAD, libraries, models and notes and reverify. TE prototype results do not establish Yageo thermal qualification. Procurement lead time does not close any engineering hold.

**Superseded, not ordered:** the prior **Uni-Royal HP122WF2201T4E**, HP12 / 2512, 2.2 kohm / 2 W / 1%, had pending `External` sourcing with no verified code. Its [SMD-SP-003 V.7 datasheet](https://www.uni-royal.cn/en/images/userfile/file/1784806233a2e6d381ea80b5d9.pdf) and [NAC Semi listing](https://store.nacsemi.com/products/detail?stock=XSJMZ0000010410), retrieved MOQ 3,600, remain historical evidence, not an order or current procurement queue. **C2791283 in commit `02661d8` is HP122WJ0472T4E, 4.7 kohm / 5%, not HP122WF2201T4E**; that mapping remains rejected and must not be renamed as PS12 history.

**Retired design/procurement history:** onsemi **1N4007G** and Vishay **MBE04140C2201FC100** preceded the PR02/BYG23T hybrid implemented after checkpoint `ad282e3`. That hybrid selected **Vishay BCcomponents PR02000202201FA100**, copper-lead PR02, 2.2 kohm / 2 W / 1% / +/-250 ppm/K, with external sourcing, no verified LCSC code and the [Vishay inventory reference](https://www.vishay.com/search?type=inv&query=PR02000202201FA100). Only **Mouser availability** was identified, not JLC allocation, purchase or reservation. **C3009798 was a rejected 220-ohm part**; the earlier PR02 **JA100** suggestion was not the selected **FA100** 1% variant. These axial selections and their forming/standoff instructions are now retired, not substitutes or current procurement tasks. Preserve any actual legacy purchase/receipt separately if confirmed; the SMT change neither invents purchases nor reallocates old parts.

Current R1/R2 metadata is **manufacturer `TE Connectivity`, MPN `35212K2FT`, C4129105, `Sourcing=LCSC`, empty `Sourcing Reference`** in the prototype BOM/CAD only. This resolves prototype resistor sourcing, not file verification or qualification. Public stock and this ledger do not set machine allocation: whole-BOM **`allocation_verified` remains `false`**. Obtain accepted PCBA-job allocation and supplier-specific attrition separately; unresolved External sourcing still blocks prototype and production publication. [REMEDIATION](REMEDIATION.md) owns file/publication results.

### Assembly And Quantity Allocation

Use the adopted [hole/pin and SMT-land controls](pcb/ASSEMBLY.md#physical-guardrails), [assembly envelopes](pcb/ASSEMBLY.md) and [DFM evidence](pcb/DFM_EVIDENCE.md) on the procurement traveller. TE 35212K2FT's **6.45 x 3.40 x 0.65 mm maximum body** fits the existing body envelope, but its recommended lands differ from the retained **alternate project lands**. Obtain supplier placement/stencil/solder acceptance for that difference, **not resistor holes, axial standoff or forming**; no exact-fit claim or geometry change follows from the TE evidence. Retained Kefa **E pin/body limits** and the independent **0.050 mm radial per-pin pattern limit for Kefa/earth GDTs** are controlled incoming requirements, not manufacturer guarantees or accepted lots. Inspect every allocated controlled-envelope part, including attrition, with uncertainty and separate pattern/body checks. W4 actual lot fit, solder fill and earth-GDT forming remain open. Apply [J_EARTH's current edge-clearance and inspection requirements](pcb/ASSEMBLY.md#earth-terminal-edge-clearance) to panel acceptance and the later complete-enclosure dry-fit; nominal in-board body/pose and courtyard margins do not approve supplier tolerances or physical fit. Physical dry-fit is not a prerequisite to generating prototype files.
...
The same-end TEST connection is **Start A/C positive, Start B DC negative, End A/B/C individually isolated**; RUN connects all start cores to T1 and all end cores to T2 using source-side ties. OFF isolates all six cable ends. See the [functional matrix](INSTALL.md#functional-matrix); no permanent field-end straps or unverified switch pin assignments. TEST/OFF mean the RF fence is inactive, not storm-safe isolation.

For source sizing, use [ELECTRICAL's named load cases](pcb/ELECTRICAL.md#2-calculated-results) and [DESIGN_BOUNDS](pcb/DESIGN_BOUNDS.md). Budget the working PSU for the full 41-board load, including the low-cable-resistance case, actual input/thermal derating and added protection/auxiliary loads. Five-board operation does not establish full-system capacity; the disconnected spare contributes none. Neither supply wattage nor a DC calculation resolves cable-fault clearing or powered-GDT recovery.

Those models cover the **TE-prototype / BYG23T population**, not retired-part values or unreviewed Yageo extrapolations. Their voltage screens are declared inputs, not enforced source limits or continuous-duty approval. TE's catalogue power/derating transfer from its reference mounting to this two-layer board remains unverified, not a demonstrated failure. SMT does not remove dissipated heat or prove cooler OneGel operation; procure and qualify for the intended population and actual material interfaces.
...
Use the [conditional cable-resistance evidence](pcb/DESIGN_BOUNDS.md#evidence-and-inputs), not an assumed reel measurement or an invented AMC numerical specification. Reconcile the **0.30 mm strand label/representative construction versus the standard's 0.26 mm maximum** and applicable supplied-lot declaration before relying on the design ceiling; do not reopen settled reel identity or colours.

Select materials for the [coastal/solar/wet-route conditions](INSTALL.md#materials-and-site), including salt and condensation, not just nominal outdoor ambient. Inspect and maintain the [whole-boundary cattle-fence prohibition](INSTALL.md#cattle-fence-prohibition); changed land use or reintroduced exposure requires reassessment before use. Keep the boundary electrically separate from supporting fence wires.
...
| Digital multimeter | Pending | De-energized continuity/diode checks and appropriate DC measurements per [INSTALL](INSTALL.md#optional-dmm-checks) and [ACCEPTANCE](ACCEPTANCE.md); not proof of GDT firing, tiny leakage or surge survival. |
...
- Confirm quantities/receipts/usable stock without inventing new orders: **user-confirmed Yageo SR2512FK-7W2K2L / C876850 order (20-day wait)**, **80 LEDs, 160 supports, 120 WAGOs**, gel, hub items, rods/clamps/wire and on-hand tap/jumper wire. Preserve the historical 110-part Uni-Royal order separately from its current cancelled/refunded status.
