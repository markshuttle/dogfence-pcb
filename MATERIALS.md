...
**Draft hardware revision 1.2.0-dev; reviewed baseline 1.1.0. Reconciled on 2026-09-07 for the user-authorized 16-part implementation after checkpoint ad282e3.**
...
The initial requirement is **five individual fully assembled boards**, including all SMT and THT work, resistor mounting and controlled GDT lead forming, with **no field soldering**. The implemented schematic/PCB/BOM specifies **16 electrical parts (six SMT, ten THT)** plus four mounting footprints. Five boards therefore need **80 electrical parts before attrition**; these are design/population counts, not accepted assemblies or purchase quantities. `pcb/BOM.csv` is the sourcing authority, not this field-material ledger. **C4, C5, W3, W1 and W4 remain open.**

**Selected design, not a purchase:** R1/R2 are **Vishay BCcomponents PR02000202201FA100**, copper-lead PR02, **2.2 kohm / 2 W / 1% / +/-250 ppm/K**. There is **no verified LCSC code**. The design BOM uses `Sourcing=External`, an empty LCSC field and the [HTTPS Vishay inventory reference](https://www.vishay.com/search?type=inv&query=PR02000202201FA100). Only **Mouser availability** has been identified; actual JLCPCB quote, accepted sourcing route and allocation are pending. **C3009798 is a 220-ohm part, not this 2.2-kohm selection.** D1-D4 are **Vishay General Semiconductor BYG23T-M3/TR / C145454**: D1/D2 series and D3/D4 negative output shunts. Five-board population is ten PR02 resistors and twenty BYG23T diodes before supplier attrition. No order or stock reservation is recorded.

**Retired design history:** onsemi **1N4007G** and Vishay **MBE04140C2201FC100** were the former series-diode/resistor selections, not current default substitutes. The previously suggested PR02 **JA100** variant is not the selected **FA100** 1% part. Preserve any actual legacy purchase/receipt separately if confirmed; this design change neither invents those purchases nor reallocates old parts. File checks validate explicit external metadata, but every public package remains blocked by unresolved external sourcing until an assigned LCSC code and sourcing update are reviewed, even if the engineering holds later close.

Use the adopted [hole/pin schedule](ORDERING.md#3-hole-via-and-solder-process), [assembly envelopes](pcb/ASSEMBLY.md) and [DFM evidence](pcb/DFM_EVIDENCE.md) on the procurement traveller. PR02's **0.83 mm maximum copper lead** uses the retained **15.24 mm pitch, 1.40 mm holes and 2.40 mm pads**; **>=1.00 mm body-to-PCB standoff is required**, not an accepted mounting/forming process. The other **E pin/body and independent 0.050 mm radial per-pin pattern limits are controlled incoming requirements, not manufacturer guarantees or accepted lots**. Inspect every allocated controlled-envelope part, including attrition, with uncertainty and separate pattern/body checks. W4 actual lot fit, solder fill and forming remain open. Budget J_EARTH's **1.30 mm pose/routed-edge overhang** in the complete enclosure fit, not merely the bare-board rectangle.
...
The [current electrical record](pcb/ELECTRICAL.md) and [DESIGN_BOUNDS](pcb/DESIGN_BOUNDS.md) now analyze PR02/BYG23T. The **35 V floor / 40.39597 V upper screen are declared inputs, not enforced hardware limits**; the older 35-37 V proposal was never implemented. The upper zero-drop/PR02 Rmin/zero-cable calculation is **1.561875985 A / 63.0935 W**, **19.047268 mA and 0.769432871 W per resistor**. The new **4.090396 mA nonuniform LED corner at 35 V** includes 5.2 V high-drop screening and 50 uA conditional clamp diversion; it is not a measured or exhaustive minimum. Nominal resistor heat remains about 0.50 W each and actual OneGel thermal testing is unperformed. These calculations neither allocate the spare's capacity nor approve procurement or continuous duty.
...
- Obtain independent allocated-part E/pattern/body reports, actual PR02 standoff/forming/solder-fill acceptance and the Ruilon metric/inch and processed-stencil resolution, including the new BYG23T SMA lands. Design limits and standard untented fabrication are not lot/process approval; no special filling concession is requested.
- Resolve exact **PR02000202201FA100 external sourcing**, assigned LCSC code and allocation through the accepted quote process; do not recycle a retired resistor code or use the 220-ohm C3009798. Request the five-board JLCPCB quote and parts/attrition/CAM/placement/solder/forming approvals only when the release gates permit. Selected CAD parts are not orders; this ledger authorizes no purchase or stock reservation.
