...
The [current electrical record](pcb/ELECTRICAL.md) and [conditional design bounds](pcb/DESIGN_BOUNDS.md) describe implementation, not protection or continuous-duty approval. The declared **35 V floor / 40.39597 V upper screen** are not enforced hardware limits; the earlier **35-37 V proposal was never implemented**. Updated PR02/BYG23T current/thermal calculations are not procurement acceptance bands or a guaranteed field-current minimum. One ordered LRS-75-36 serves TEST; its disconnected spare adds no capacity. The user's prohibition on energized cattle fencing covers the **whole 4 km boundary cable, every station and the hub**; the old 0.5 m / 300 m parallel case is withdrawn, not a current energizer-on/off qualification requirement. Maintain the restriction and reassess before use if the hazard returns; no numerical safe separation or zero induction/lightning claim follows.
...
Use headless KiCad 9 (tested baseline 9.0.7). `make check` retains evidence and refreshes the CPL reference only after all file/artifact checks pass, **not an upload package**. It explicitly validates `Sourcing=External`, empty LCSC metadata and a matching HTTPS sourcing reference without treating that as allocation. **`make all` and every direct export alias remain blocked by C4/C5/W3/W1/W4 and independently by unresolved PR02 external sourcing**, even if the engineering holds later close. An assigned LCSC code and reviewed sourcing update are required before public publication; no JLC procurement route is promised before quote/allocation. Do not clear holds or invent a code to obtain ZIPs. Private draft exports remain under `tmp/manufacturing/runs/attempt-*`. Serial/parallel builds use a lock and quarantine stale public outputs. `make clean` removes owned runs/outputs only; preserve useful run evidence first and run clean separately.
...
| Assembly | All **six SMT and ten THT parts per board**, top-side, fitted and soldered, with controlled PR02 standoff and axial GDT lead forming. Confirm against the accepted design before quote acceptance. |
...
| Solder process | Explicit acceptance of heavy-copper soldering, Ruilon and BYG23T SMT paste volume, THT fillets/barrel wetting, PR02 >=1.00 mm body standoff and GDT forming/standoff inspection. No field soldering or forced insertion/reaming to repair fit. |
...
- For the retained Kefa, PR02 and GDT holes, require `nominal finished hole - 0.08 - maximum pin envelope - 0.241421 >= 0.10 mm`. The separate **0.241421 mm diametral position budget** includes **+/-0.05 mm hole position on each axis** and an independently inspected **0.050 mm radial per-pin part-pattern limit after common rigid alignment**, through the insertion length. It is additional to the 0.10 mm diametral assembly allowance, not a replacement for it. Use rectangular-pin diagonals, not nominal widths.
...
- Agree reflow and THT soldering processes for the heavy copper without unauthorized thermal-relief narrowing. PR02 requires **>=1.00 mm body-to-PCB standoff** at the retained 15.24 mm formed pitch; its 12.0 mm maximum L2 / 3.9 mm maximum body diameter and 0.83 mm maximum copper lead are not actual forming or thermal acceptance. The GDT_AC drawing retains **at least 2.00 mm gap under the whole raised body/electrode/lead span**, measured above the highest finished PCB top surface before gel. Its development target is **body underside 2.50 +/-0.25 mm and complete assembled height <=8.50 mm**. These limits do not approve actual seal support, bend radius, springback or forming/solder process.
...
| R1, R2 / Vishay PR02000202201FA100 | **1.40 / 2.40**, retained | Copper lead **0.83 maximum**, from 0.78 +/-0.05; `PR02_Cu_P15.24mm`, not the FeCu variant. |
...
Independently inspect every allocated controlled-envelope part for the five prototypes, including attrition, with measurement uncertainty and pin-pattern/body checks. PR02's published lead maximum replaces the retired MBE's E limit, not actual mounting/lot acceptance. The old 1N4007G 1.10/2.20 mm hole/pad and diode-forming treatment are historical; the current diodes are SMA. Larger holes do not establish free insertion, lot yield or adequate solder fill. **W4 remains open for actual lot/pattern/body fit and process acceptance.**
...
The implemented circuit/BOM has **16 electrical parts per board: six SMT + ten THT, all on top**, excluding four mechanical hole footprints. D1/D2 replace the old through-hole rectifiers, D3/D4 add negative-voltage shunts, and PR02 replaces MBE0414. Protected rails, vias, earth geometry and LED terminals remain unchanged; no folded takeoff or additional via is implemented. The quantities below are **design population only**, not native-check results, purchase quantities or stock reservations.
...
| R1, R2 | **Vishay BCcomponents PR02000202201FA100** | **External; no verified LCSC code** | 2 | 10 |
...
PR02 is **2.2 kohm / 2 W / 1% / +/-250 ppm/K, copper leads**, not a 1.3 W FeCu part. Its design BOM explicitly records `Sourcing=External`, an empty LCSC code and the [HTTPS Vishay inventory reference](https://www.vishay.com/search?type=inv&query=PR02000202201FA100). Only **Mouser availability** has been identified, not JLCPCB stock or allocation. Obtain acceptance of the exact source/packaging and quote route, then an assigned LCSC code and reviewed metadata before public publication. Do not reuse the retired MBE sourcing code or **C3009798, which is 220 ohms**, for this 2.2-kohm part. No purchase or reservation has been made. The old onsemi 1N4007G/MBE0414 remain design history, not default substitutes; actual procurement history stays in MATERIALS.
...
| D1, D2 | Centres **(132.50,104.50)/(132.50,140.50) mm**, 0 degrees, cathode bands right toward LED positive. Check actual pad/net mapping and the BYG23T model, not the retired through-hole geometry. |
| D3, D4 | Centres **(135.00,99.00)/(135.00,146.00) mm**, 180 degrees, **cathodes left to LED positive; anodes right to B**. They use the same local cathode-right footprint rotated, not matching global diode orientations. |
| R1, R2 | Centres **(116.00,104.50)/(116.00,140.50) mm**, 0 degrees; `PR02_Cu_P15.24mm`, retained pad centres. Nonpolar; >=1.00 mm body-to-PCB standoff requires physical process acceptance. |
...
- Selected parts: [Vishay PR01/PR02/PR03, 28729, 08-Jul-2025](https://www.vishay.com/docs/28729/pr010203.pdf), [BYG23T-M3, 89429, 25-Feb-2020](https://www.vishay.com/docs/89429/byg23t.pdf); source conditions and limitations are recorded in [DESIGN_BOUNDS](pcb/DESIGN_BOUNDS.md).
