...
environmental research. The authorized circuit/layout is the 16-part HP12/BYG23T
...
| :--- | :--- |
| **HP12 sourcing** | Resolve the exact **HP122WF2201T4E, 2.2 kohm / 2 W / 1%** mapping before publication; ten fitted parts plus supplier-specified attrition need accepted allocation before order. **C2791283 is HP122WJ0472T4E, 4.7 kohm / 5%, and is rejected.** Keep sourcing pending with no guessed code. No resistor lead forming or insertion-hole approval is needed for this SMT part. |
...
| **SMT and THT process** | Accept the manufacturer HP12 lands and selected metric Ruilon/SMA lands, resolve Ruilon's 4.00 mm/0.165-inch conflict, and approve the processed stencil and heavy-copper solder/inspection process. |
...
blocks publication. Supplier responses above are order tasks, not a demand for
...
The [current electrical record](pcb/ELECTRICAL.md) and [conditional design bounds](pcb/DESIGN_BOUNDS.md) describe implementation, not protection or continuous-duty approval. The declared **35 V floor / 40.39597 V upper screen** are not enforced hardware limits; the earlier **35-37 V proposal was never implemented**. Updated HP12/BYG23T current/thermal calculations are not procurement acceptance bands or a guaranteed field-current minimum. One ordered LRS-75-36 serves TEST; its disconnected spare adds no capacity. The user's prohibition on energized cattle fencing covers the **whole 4 km boundary cable, every station and the hub**; the old 0.5 m / 300 m parallel case is withdrawn, not a current energizer-on/off qualification requirement. Maintain the restriction and reassess before use if the hazard returns; no numerical safe separation or zero induction/lightning claim follows.
...
Use headless KiCad 9 (tested baseline 9.0.7). `make check` retains evidence and refreshes the CPL reference only after all file/artifact checks pass, **without publication**. Pending sourcing must use `Sourcing=External`, empty LCSC metadata and a matching HTTPS reference; file parity does not verify allocation. **`make gerbers` / `make prototype` publish FILE-VERIFIED PROTOTYPE ONLY packages with all five holds deferred, but unresolved HP12 external sourcing still blocks publication.** Resolve the exact code and reviewed metadata, never guess a code or clear qualification holds to obtain ZIPs.
...
| Assembly | All **eight SMT and eight THT parts per board**, top-side, fitted and soldered, with HP12 SMT resistors and controlled axial GDT lead forming. Confirm against the accepted design before quote acceptance. |
...
| Solder process | Explicit acceptance of heavy-copper soldering, HP12/Ruilon/BYG23T SMT paste volume, THT fillets/barrel wetting and GDT forming/standoff inspection. No axial resistor forming, field soldering or forced insertion/reaming to repair fit. |
...
- R1/R2 use the manufacturer **HP12 lands: 1.35 x 3.70 mm rectangles at local X=+/-3.125 mm**, zero additional mask/paste margins, **6.45 x 3.40 mm maximum body and 8.10 x 4.20 mm courtyard**. Review all four resistor apertures and the actual stencil/reflow process; do not substitute a generic 2512 land pattern or restore resistor holes/standoff/forming.
...
| R1, R2 | **Uni-Royal HP122WF2201T4E** | **Pending; no verified LCSC code** | 2 | 10 |
...
HP12 is **2512 SMT, 2.2 kohm / 2 W / 1% / +/-100 ppm/C referenced to 25 C**. **C2791283 maps to HP122WJ0472T4E, 4.7 kohm / 5%, not HP122WF2201T4E.** Until the exact intended mapping is reviewed, keep `Sourcing=External`, an empty LCSC code and an exact HTTPS sourcing reference matched to the sources. This unresolved component identity/allocation route blocks **both** publication modes, independently of deferred engineering holds. A valid reviewed code does not reserve stock: obtain the exact packaging, accepted quote route and actual allocation before order. No purchase or reservation has been made. PR02's earlier Mouser sourcing lead and the old onsemi/MBE selections remain history in MATERIALS, not current procurement/forming tasks or default substitutes.

The BOM's [exact NAC Semi reference](https://store.nacsemi.com/products/detail?stock=XSJMZ0000010410)
identifies the intended part, but the retrieved listing specifies **MOQ 3,600**.
It is not an accepted ten-part prototype quote or JLCPCB allocation. Public
search-page limitations also prevent treating an empty search as proof that
JLCPCB cannot supply it; obtain the actual exact-part detail or supplier response.
...
| R1, R2 | Origins **(120.00,104.50)/(120.00,140.50) mm**, 0 degrees; pads at **X=116.875/123.125**. Nonpolar HP12 SMT, using the manufacturer lands above; no insertion holes, axial standoff or lead forming. |
...
- Selected parts: [Uni-Royal HP Series, SMD-SP-003 V.7, 08-Jan-2026](https://www.uni-royal.cn/en/images/userfile/file/1784806233a2e6d381ea80b5d9.pdf), [BYG23T-M3, 89429, 25-Feb-2020](https://www.vishay.com/docs/89429/byg23t.pdf); source conditions and limitations, including retired PR02 evidence, are recorded in [DESIGN_BOUNDS](pcb/DESIGN_BOUNDS.md). HP12's 2 W rating at 70 C ambient derates to zero at 155 C; SMT heat flow does not remove the loss or prove cooler continuous operation.
