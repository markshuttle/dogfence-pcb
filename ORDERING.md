...
**Draft hardware revision 1.2.1-dev; reviewed baseline 1.1.0. File-verified prototype artifacts are authorized; uploads and orders are not. Production/field holds remain in force.**
...
- Exact parts and local footprints against the [BOM](pcb/BOM.csv) and [reviewed component index](pcb/ASSEMBLY.md#reviewed-components), not retired selections or stock-library substitutes. D1-D4, R1/R2 and all three inter-core GDTs are SMT; GDT_AC's former axial part and two holes are retired. The retained resistor lands are an alternative to TE's recommendation requiring supplier acceptance under section 3.
- The adopted [hole/pin/pattern limits](pcb/ASSEMBLY.md#hole-fit-evidence), [body envelopes](pcb/ASSEMBLY.md#body-and-courtyard-evidence), real courtyards, reproducible local libraries, legend and earth-GDT forming drawing. Actual allocated-lot fit and solder/process acceptance are supplier order tasks, not file-check results. A standard-looking footprint name or matching C-code is not a mechanical match.
- All three SMT GDTs' mask/paste geometry around the protected vias, native signed-Y placement, and complete population/net parity. Verify the [layer-specific rail/crossing guardrails](pcb/ASSEMBLY.md#physical-guardrails): **exactly 6.00 mm rear B main width**, approved front-B stop and rear-return move, and **>=3.00 mm different-core A/B/C clearance on B.Cu only**. Both native and independent geometry checks are required; do not impose that inter-core rule on F.Cu's reviewed SMT gaps.
- Verify the approved **1.2.1-dev westward placement** and its separate **>=3.00 mm unlike-net inter-GDT pad / AC-pad-to-front-B copper controls**. Accept processed clearances and placement against [the current evidence](pcb/DFM_EVIDENCE.md#westward-clearance-evidence), not the old coordinates or a blanket 3 mm front-layer claim.
...
| Assembly | All **nine SMT and seven THT parts per board**, top-side, fitted and soldered, with three SMT inter-core GDTs and controlled lead forming for the three axial earth GDTs. Confirm against the accepted design before quote acceptance. |
...
| Solder process | Explicit acceptance of heavy-copper soldering, 2512 / all three Ruilon GDT / BYG23T SMT paste volume, THT fillets/barrel wetting and earth-GDT forming/standoff inspection. No axial resistor or GDT_AC forming, field soldering or forced insertion/reaming to repair fit. |
...
- All retained connector/earth-GDT component holes remain open for insertion and no via is an insertion pad; SMT resistors, diodes and inter-core GDTs have **no component holes**. Verify GDT_AC's former two holes are absent. Obtain written CAM agreement not to reduce protected via drills; ordinary via guidance does not guarantee a finished 1.00 mm diameter/tolerance. Any required finished-via minimum is a separate unresolved supplier fact, not assumed special processing.
- Use the [selected metric Ruilon lands](pcb/ASSEMBLY.md#smt-via-relocation) for **GDT_AB, GDT_BC and GDT_AC**. The printed **4.00 mm centre spacing is selected**, not an inner-gap dimension; the conflicting **0.165-inch** entry is not an approved erratum. **W1 remains open for that contradiction and actual placement/processed-stencil/solder-profile acceptance at all three positions.**
- Compare complete drill circles, exposed annuli and actual mask/paste, not just pad centres. [Nominal aperture gaps](pcb/ASSEMBLY.md#smt-via-relocation) are not registration/solder-spread allowances. Recheck all Ruilon, 2512 and BYG23T apertures in fresh native and processed data; none may overlap a protected via hole or annulus. Preserve the **2.80 mm land/mask/paste gap** and separate **1.80 mm connected-copper gap** beneath each SMT inter-core GDT; do not extend the full-width stubs blindly to the pad centres. Customer F.Paste does not approve the assembler's processed stencil.
- Accept the **0.25 mm J_IN-to-AB/BC courtyard gap** and the relocated vias under J_IN's conservative body projection. Inspect actual access, seating, solder ingress/beads and hidden cleanliness against the [current assembly controls](pcb/ASSEMBLY.md#smt-via-relocation); a clear courtyard drawing does not prove physical fit or solder-process acceptance.
...
- For the retained Kefa and earth-GDT holes, require `nominal finished hole - 0.08 - maximum pin envelope - 0.241421 >= 0.10 mm`. The separate **0.241421 mm diametral position budget** includes **+/-0.05 mm hole position on each axis** and an independently inspected **0.050 mm radial per-pin part-pattern limit after common rigid alignment**, through the insertion length. It is additional to the 0.10 mm diametral assembly allowance, not a replacement for it. Use rectangular-pin diagonals, not nominal widths.
...
- Agree reflow and THT soldering processes for the heavy copper without unauthorized thermal-relief narrowing. Obtain dimensioned [forming approvals for the three axial earth GDTs](pcb/ASSEMBLY.md#axial-forming-approval), including seal support, bend radius, springback, standoff and complete assembled height. **GDT_AC is SMT, with no lead-forming or overpass-height-gauge requirement**; accept its actual seating/fillets and supplier model under [Placement And Polarity](pcb/ASSEMBLY.md#placement-and-polarity). Check the front-B stop and continued connection to GDT_B_E's input PTH through the rear copper. The through-board crossing uses the FR-4 core, not the retired air-gap overpass; laminate, solder mask and gel do not supply impulse-insulation approval.
...
The BOM-derived fitted population is **16 electrical parts per board: nine SMT + seven THT, all on top**, excluding four mechanical hole footprints. Five boards require **45 SMT + 35 THT parts before attrition**. These are quote population counts, not native-check results, purchase quantities or stock reservations.

| References | Assembly | Per board | Five-board population |
| :--- | :--- | ---: | ---: |
| D1-D4 | SMT | 4 | 20 |
| R1, R2 | SMT | 2 | 10 |
| GDT_AB, GDT_BC, GDT_AC | SMT | 3 | 15 |
...
Whole-BOM **`allocation_verified` remains `false`**. Obtain accepted allocation of all **80 fitted parts plus line-specific attrition**, including ten TE resistors and fifteen SMT GDTs, before ordering boards. The reused Ruilon code does not establish new stock or allocation approval; [MATERIALS' GDT quantity/history record](MATERIALS.md#gdt-procurement-and-history) distinguishes the additional fitted requirement from purchases or cancellations. Record allocated quantities, leftovers and test/spare use in [MATERIALS](MATERIALS.md#assembly-and-quantity-allocation); purchases and this population table do not establish receipt, inspection or allocation.
...
| GDT_AC | Same top-side Ruilon SMT pattern as GDT_AB/BC, internally north-south at the approved westward origin/0-degree angle; pad 1 A north, pad 2 C south. Review the actual SMT model/seating, not the retired Bencent model or axial height gauge. B crosses on the rear layer. |
