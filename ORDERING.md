...
environmental research. The authorized circuit/layout is the 16-part 2512 SMT /
BYG23T hybrid (TE Connectivity 35212K2FT / C4129105 prototype BOM only;
Yageo SR2512FK-7W2K2L / C876850 ordered for future production, not yet integrated);
unresolved performance holds remain recorded, not silently closed or
evidence that a different circuit is already required.

| Next board decision | Exact response/evidence needed |
| :--- | :--- |
| **2512 resistor job allocation** | The user selected **TE Connectivity 35212K2FT / [C4129105](https://jlcpcb.com/partdetail/C4129105), 2.2 kohm / 2 W / 1%**, for the prototype order (in stock at JLCPCB). The user has also ordered **Yageo SR2512FK-7W2K2L / [C876850](https://jlcpcb.com/partdetail/C876850)** with an advised 20-day wait for future production, not confirmed receipt or allocation. The prior Uni-Royal PS12 order was cancelled/refunded due to stockout; its 110-part history remains in MATERIALS. Accept ten fitted TE parts plus supplier-specified attrition for the five prototypes before order. No resistor lead forming or insertion-hole approval is needed. |
...
blocks publication, but in-stock prototype sourcing is resolved by
TE Connectivity 35212K2FT / C4129105. Supplier responses above are order tasks, not a demand for
...
- The authorized **16-part SMT/THT hybrid and six-end hub matrix**, with **C4 negative-voltage/transient qualification, C5 PSU/fuse/GDT recovery, W3 actual continuous thermal behavior and W1/W4 process/fit acceptance still open**. Installation-polarity LED damage is user-accepted, not a required immunity feature. D3/D4 are output shunts after D1/D2, not extra series parts or qualified forward pulse-current regulators. Historical active candidates are not required purchases.
- Exact **Vishay General Semiconductor BYG23T-M3/TR / C145454** for D1-D4 and selected **TE Connectivity 35212K2FT / C4129105** for both R1/R2 in the prototype BOM only (with Yageo SR2512FK-7W2K2L / C876850 ordered with 20-day wait; Uni-Royal PS12 cancelled/refunded). The old 1N4007G/MBE0414/PR02 are retired; prior HP122WF2201T4E pending External sourcing is superseded, not ordered. D1-D4 and R1/R2 use unchanged SMT lands, with no resistor/diode insertion holes or axial resistor standoff/forming. The resistor lands are an alternate project pattern requiring supplier acceptance, not TE's recommended pattern.
...
The [current electrical record](pcb/ELECTRICAL.md) and [conditional design bounds](pcb/DESIGN_BOUNDS.md) describe implementation, not protection or continuous-duty approval. The declared **35 V floor / 40.39597 V upper screen** are not enforced hardware limits; the earlier **35-37 V proposal was never implemented**. Current/thermal calculations are not procurement acceptance bands or a guaranteed field-current minimum. One ordered LRS-75-36 serves TEST; its disconnected spare adds no capacity. The user's prohibition on energized cattle fencing covers the **whole 4 km boundary cable, every station and the hub**; the old 0.5 m / 300 m parallel case is withdrawn, not a current energizer-on/off qualification requirement. Maintain the restriction and reassess before use if the hazard returns; no numerical safe separation or zero induction/lightning claim follows.
...
- R1/R2 retain **alternate project lands: 1.35 X x 3.70 Y mm rectangles at local X=+/-3.125 mm, 4.90 mm inner gap / 6.25 mm pitch**. TE recommends **1.50 X x 3.00 Y mm, 5.00 mm inner gap / 6.50 mm pitch**, so these are not manufacturer-exact lands. TE's **6.45 x 3.40 x 0.65 mm maximum body** fits the retained body envelope; this does not approve placement or solder joints. The **8.10 x 4.20 mm courtyard** and project-selected zero additional mask/paste margins remain unchanged. Obtain explicit supplier placement, processed-stencil and solder/reflow acceptance for the alternate lands under the existing hold in [ASSEMBLY](pcb/ASSEMBLY.md). No layout change, generic-footprint substitution or resistor holes/standoff/forming is authorized by this correction.
...
The authorized circuit/BOM has **16 electrical parts per board: eight SMT + eight THT, all on top**, excluding four mechanical hole footprints. D1/D2 replace the old through-hole rectifiers and D3/D4 add negative-voltage shunts; the prior axial resistor selections are retired. The TE selection retains the existing SMT geometry. **The prototype BOM selects TE Connectivity 35212K2FT / C4129105 for R1/R2 only; Yageo is not integrated into the BOM/CAD.** Protected rails, vias, earth geometry and LED terminals remain unchanged; no folded takeoff or additional via is implemented. The quantities below are **design population only**, not native-check results, purchase quantities or stock reservations.
...
| R1, R2 | **TE Connectivity 35212K2FT** | **[C4129105](https://jlcpcb.com/partdetail/C4129105)** | 2 | 10 |
...
TE 3521 is **2512 SMT, 2.2 kohm / 2 W / 1% / +/-100 ppm/C, AEC-Q200 qualified**. Current prototype BOM/CAD metadata is **manufacturer `TE Connectivity`, MPN `35212K2FT`, C4129105, `Sourcing=LCSC`, empty `Sourcing Reference`**. It is in stock at JLCPCB as an Extended Part. Sourcing resolves in-stock prototype availability, not the whole-BOM allocation review: **`allocation_verified` remains `false`**. The user confirms **Yageo SR2512FK-7W2K2L / C876850 ordered with an advised 20-day wait**, not receipt, inspection or allocation. The prior Uni-Royal PS12 order was cancelled/refunded due to stockout. Obtain packaging, order-specific attrition and accepted allocation of ten fitted TE resistors for the five prototypes before ordering boards. [MATERIALS](MATERIALS.md) preserves the distinct purchase histories and quantities; no agent upload or order is authorized.
...
| R1, R2 | Origins **(120.00,104.50)/(120.00,140.50) mm**, 0 degrees; pads at **X=116.875/123.125**. Nonpolar TE 3521 on the retained alternate project lands above, pending supplier placement/stencil/solder acceptance; no insertion holes, axial standoff or lead forming. |
...
For later production, separately review **Yageo SR2512FK-7W2K2L / C876850** against its own manufacturer/rating, land-pattern, thermal, pulse and process evidence. Update the controlled BOM/CAD, libraries, models and notes for the accepted production part and reverify before production release; an order does not create an approved substitution or an automatic BOM variant. **TE prototype test results do not qualify Yageo thermal behavior.** Close the applicable production/field gates for the intended configuration and settle prototype/spare allocation in MATERIALS. Requote the **actual remaining 41-station requirement**, approved hardware revision, parts, attrition, panelization and process. Reusing order history does not authorize old Gerbers, quantities, substitutes, gold thickness or via treatment. Revalidate files and both manufacturer approvals before any reorder.
...
- Prototype parts: [TE Connectivity 3521 Series, 9-1773463-5, Rev G, 02/2025](https://www.te.com/commerce/DocumentDelivery/DDEController?Action=showdoc&DocId=Data+Sheet%7F9-1773463-5%7FG1%7Fpdf%7FEnglish%7FENG_DS_9-1773463-5_G1.pdf), [JLCPCB C4129105 identity](https://jlcpcb.com/partdetail/C4129105), and [BYG23T-M3, 89429, 25-Feb-2020](https://www.vishay.com/docs/89429/byg23t.pdf). [DFM_EVIDENCE](pcb/DFM_EVIDENCE.md) controls the TE land-pattern difference; [DESIGN_BOUNDS](pcb/DESIGN_BOUNDS.md) controls thermal/TCR assumptions and voltage limits. TE page 3's four-layer, 2 oz outer / 4 oz inner reference is not this two-layer board: transferring its 2 W P70/derating is conditional, not proof of two-layer failure or a new four-layer fabrication requirement. The 25 C resistance reference is an engineering assumption, and PS12 overload/pulse data must not be transferred to TE.
- Production order identity: [Yageo SR2512FK-7W2K2L / JLCPCB C876850](https://jlcpcb.com/partdetail/C876850), user-confirmed order with 20-day wait, not receipt/allocation or completed manufacturer/thermal review. Prior Uni-Royal PS12/HP12 and PR02 sources remain in the controlled notes and MATERIALS as history, not current prototype selections.
