...
The authorized **16-part hybrid uses two Uni-Royal PS12 2512 SMT resistors and four BYG23T SMA diodes**, including two negative-voltage output shunts. The user selected **PS122WF2201T4E / C2793873, 2.2 kohm / 2 W / 1%, for both R1/R2 in prototype and production under one BOM**, and reports **110 ORDERED into their JLCPCB parts library**. This is not receipt, inspection or allocation to a PCBA job; see [MATERIALS](MATERIALS.md). Implementation is not assembled hardware or pulse/thermal qualification; **C4, C5, W3, W1 and W4 remain open**, deferred only for the prototype-artifact scope below.
...
| :--- | :--- |
| A indicator | WIRE_A -> R1 (PS12, 2.2 kohm / 2 W) -> D1 (BYG23T) -> LED_A_POS -> external LED A -> WIRE_B |
| C indicator | WIRE_C -> R2 (PS12, 2.2 kohm / 2 W) -> D2 (BYG23T) -> LED_C_POS -> external LED C -> WIRE_B |
...
The updated analysis includes PS12 tolerance/TCR, a higher BYG23T forward-drop screen and conditional clamp leakage. The forward-only core model still represents healthy normal TEST: D3/D4 add no forward B-to-A/C path. It does not establish reverse leakage, RF behavior, surge survival, thermal limits or daylight visibility. Neither the 8.04 mA nor the illustrative 6.21 mA comparison is an accepted worst-case current.
...
At the nominal source-end comparison point, each 2.2 kohm resistor still dissipates about **0.50 W**, approximately **1 W per board in the two resistors alone**. The selected **Uni-Royal PS122WF2201T4E** is **PS12 / 2512 SMT, 2 W at 70 C ambient, 1%, +/-100 ppm/C referenced to 25 C** (TCR test endpoints -55/+125 C), derating to zero at 155 C ambient per [SMD-SP-007 V.7, 08-Jan-2026](https://www.uni-royal.cn/en/images/userfile/file/1784854235b7c79f8d8a205c5d.pdf). Its chip/termination/PCB heat path may help heat rejection, but does not lower electrical loss or prove cooler OneGel operation. PR02 hot-spot, thermal-resistance and standoff data do not apply. W3 remains open.

The stdlib analysis is reproducible with `python3 -B scripts/analyze_limits.py` and `python3 -B scripts/design_bounds.py`. It models all three cores, exercises **280 clean-cut cases**, and screens shorts and conditional ignited-GDT paths. [DESIGN_BOUNDS](pcb/DESIGN_BOUNDS.md) records the selected PS12 resistance envelope, upper current/power screens and ambient derating. The numerical resistor screens remain **0.501018 W nominal / 0.756803 W upper**. These are calculations, not a guaranteed source maximum, self-heating solution or physical test result.

PS12's family ceilings are **500 V working / 1000 V overload**, but the power/resistance limits reduce this 2 W / 2.2 kohm part to **66.33 V working / 165.83 V for the specified 5 s overload**. These are component conditions, not a 500 V system rating. The PS datasheet's **page 6 one-pulse power and voltage curves** are not repetitive-pulse or assembled-board qualification; use the controlled electrical evidence for their scope.

The [updated design bounds](pcb/DESIGN_BOUNDS.md#normal-current) retain conditional cable/contact assumptions and a declared **35 V feed floor / 40.39597 V upper screen**, neither enforced nor guaranteed. Nonuniform PS12 loads, a **5.2 V combined forward-drop screen** and **50 uA conditional clamp diversion** give named current corners, not an exhaustive minimum or measured brightness; the loose comparison floor is not predicted installed brightness. Series-diode reverse leakage and physical OFF-state glow are not solved by the ideal cut model. Older PR02/MBE results and the **35-37 V proposal are historical**. Qualify the real normal source range and closed OneGel assembly for production/field use; no active 37 V cutoff or historical 40-part circuit is imposed as a prototype prerequisite.
...
| R1, R2 | **Uni-Royal PS122WF2201T4E** | **[C2793873](https://jlcpcb.com/partdetail/C2793873)** | PS12 / 2512 SMT, 2.2 kohm, 2 W P70, 1%, +/-100 ppm/C at 25 C reference; same selection for prototype and production. No resistor insertion holes or lead forming. Actual solder process and OneGel thermal acceptance remain open. |
...
| R1, R2 | Vishay PR02000202201JA100, historical PR02 suggestion; both it and the former 1% FA100 selection are **retired, not the selected PS12 SMT part** |
...
| Resistors | R1/R2 origins (120.00,104.50)/(120.00,140.50), 0 degrees, pad centres X=116.875/123.125. PS12 matches the existing 1.35 x 3.70 mm rectangular lands at local X=+/-3.125; maximum body 6.45 x 3.40 mm, maximum height 0.65 mm. The 8.10 x 4.20 mm courtyard and project-selected zero additional mask/paste margins remain unchanged. No resistor holes, axial standoff or forming; no folded takeoff or additional via. |
...
The selected hole/pad schedule is **KF128 2.00/3.20, KF129 2.00/2.80, B5G470L 1.40/2.80, earth GDTs 1.50/3.00 mm**. Minimum nominal ring is 0.40 mm; earth GDT maximum lead is 1.05 mm. Other controlled pin envelopes and the separate hole-position/formed-pattern budget remain in [fit evidence](pcb/ASSEMBLY.md#hole-fit-evidence). Actual allocated THT parts still must pass free-insertion, GDT forming/standoff and solder-process inspection; supplier CAD or nominal pitch is not that acceptance. PS12 resistor acceptance concerns its SMT lands, body, placement and solder process, not insertion or axial forming.
...
`make check` performs file verification and refreshes the generated CPL reference only after all file/artifact checks pass, **without publication**. `make gerbers` / `make prototype` run the same complete checks and may publish **FILE-VERIFIED PROTOTYPE ONLY** packages with all five ledger holds explicitly deferred, not closed. **The selected PS12/C2793873 resolves the former resistor External blocker, not any file or qualification gate**; unresolved external sourcing still blocks both modes. Use [REMEDIATION](REMEDIATION.md) for the latest revision-specific results, not prior HP12 checks as evidence of a new native PASS or publication.
