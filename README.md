...
The authorized **16-part hybrid uses two 2512 SMT resistors and four BYG23T SMA diodes**, including two negative-voltage output shunts. For the **prototype BOM only**, the user authorized **TE Connectivity 35212K2FT / [C4129105](https://jlcpcb.com/partdetail/C4129105)** (2.2 kohm / 2 W / 1%, AEC-Q200, in stock at JLCPCB). The user has also ordered **Yageo SR2512FK-7W2K2L / [C876850](https://jlcpcb.com/partdetail/C876850)** with an advised 20-day wait for future production builds, not confirmed receipt or job allocation. The prior Uni-Royal PS12 order was cancelled/refunded by JLCPCB due to stockout; [MATERIALS](MATERIALS.md) preserves its historical 110-part order separately. Implementation is not assembled hardware or pulse/thermal qualification; **C4, C5, W3, W1 and W4 remain open**, deferred only for the prototype-artifact scope below.

The Yageo order is not an approved substitution or a second implemented BOM. Before production release, separately review its manufacturer, land-pattern, thermal and pulse evidence and update the controlled BOM/CAD, libraries, models and notes, then reverify the release. TE prototype results do not establish Yageo thermal qualification.

[REMEDIATION.md](REMEDIATION.md) records the agreed requirements, issue evidence, latest revision-specific verification results, and release gates. The design sources are the working schematic, PCB, project settings/rules, and libraries under `pcb/`, with reviewed sourcing in `pcb/BOM.csv`. Generated `build/` files, backup archives, and `tmp/` copies are not design authorities. [Electrical analysis](pcb/ELECTRICAL.md) and [controlled assembly notes](pcb/ASSEMBLY.md) distinguish implemented changes from unresolved protection, fit and process decisions. `pcb/verification.json` retains the engineering hold ledger; prototype publication records those holds without closing them. Revision labels and documentation changes alone do not establish hardware qualification.
...
| Implemented circuit path | Connection |
| :--- | :--- |
| A indicator | WIRE_A -> R1 (2512 SMT, 2.2 kohm / 2 W) -> D1 (BYG23T) -> LED_A_POS -> external LED A -> WIRE_B |
| C indicator | WIRE_C -> R2 (2512 SMT, 2.2 kohm / 2 W) -> D2 (BYG23T) -> LED_C_POS -> external LED C -> WIRE_B |
| Negative output shunts | D3 cathode to LED_A_POS, D4 cathode to LED_C_POS; both anodes to WIRE_B. These BYG23T diodes are across the LED connectors, **after** D1/D2, not additional series components. |
...
The **historical zero-leakage, forward-only comparison**, not a measurement, uses 41 stations, an ideal 36 V source, 27.6 ohms per 4 km core, 2.2 kohm per LED branch, and 2.8 V combined LED/rectifier forward drop. It is retained for comparison, not as measured BYG23T forward voltage at indicator current:
...
The updated analysis includes 2512 SMT tolerance/TCR, a higher BYG23T forward-drop screen and conditional clamp leakage. The forward-only core model still represents healthy normal TEST: D3/D4 add no forward B-to-A/C path. It does not establish reverse leakage, RF behavior, surge survival, thermal limits or daylight visibility. Neither the 8.04 mA nor the illustrative 6.21 mA comparison is an accepted worst-case current.
...
At the nominal source-end comparison point, each 2.2 kohm resistor still dissipates about **0.50 W**, approximately **1 W per board in the two resistors alone**. The active prototype selection **TE Connectivity 35212K2FT** is **CGS 3521 / 2512 SMT, 2 W at 70 C ambient, 1%, +/-100 ppm/C at 2.2 kohm**, with -55 to +155 C operation and derating to zero at 155 C ambient per [Data Sheet 9-1773463-5, Rev G, 02/2025](https://www.te.com/commerce/DocumentDelivery/DDEController?Action=showdoc&DocId=Data+Sheet%7F9-1773463-5%7FG1%7Fpdf%7FEnglish%7FENG_DS_9-1773463-5_G1.pdf). The earlier 03/2023 attribution has no archived supporting copy in the review evidence. TE page 3 uses a **four-layer PCB with 2 oz outer / 4 oz inner copper** as its mounting/thermal reference; this project has **two layers, 2 oz each**. Transfer of P70/derating is conditional, not proof that two layers fail. SMT heat flow does not lower electrical loss or prove cooler OneGel operation; PR02 hot-spot, thermal-resistance and standoff data do not apply. W3 remains open.

The stdlib analysis is reproducible with `python3 -B scripts/analyze_limits.py` and `python3 -B scripts/design_bounds.py`. It models all three cores, exercises **280 clean-cut cases**, and screens shorts and conditional ignited-GDT paths. [DESIGN_BOUNDS](pcb/DESIGN_BOUNDS.md) controls the resistance envelope, upper current/power screens and conditional ambient derating. Its retained **25 C resistance reference and -30..125 C linear screen are engineering assumptions**; neither a 25 C reference nor -55/+125 C TCR test endpoints are verified TE facts. The numerical resistor screens remain **0.501018 W nominal / 0.756803 W upper**, conditional on the declared inputs, not a guaranteed source maximum, self-heating solution or physical test result.

TE 3521's family ceilings are **250 V working / 500 V overload**. The catalogue power/resistance arithmetic gives **66.33 V working at 2 W / 2.2 kohm**, conditional on applicable mounting/derating, not a 250 V system rating. The cited TE sheet does **not** establish a five-second `2.5 * RCWV` overload test. That test and the PS12 one-pulse power/voltage curves belong to historical PS evidence, not TE pulse qualification; use the controlled electrical notes for the remaining limits.

The [updated design bounds](pcb/DESIGN_BOUNDS.md#normal-current) retain conditional cable/contact assumptions and a declared **35 V feed floor / 40.39597 V upper screen**, neither enforced nor guaranteed. Nonuniform 2512 SMT loads, a **5.2 V combined forward-drop screen** and **50 uA conditional clamp diversion** give named current corners, not an exhaustive minimum or measured brightness; the loose comparison floor is not predicted installed brightness. Series-diode reverse leakage and physical OFF-state glow are not solved by the ideal cut model. Older PR02/MBE results and the **35-37 V proposal are historical**. Qualify the real normal source range and closed OneGel assembly for production/field use; no active 37 V cutoff or historical 40-part circuit is imposed as a prototype prerequisite.

**Protection holds remain open:**

- **C4, implemented but unqualified:** D3/D4 provide negative-voltage shunt paths, not positive regulation, forward pulse-current limiting or reversed-flying-lead immunity. BYG23T's **75 ns reverse recovery is not forward-clamp response time**. Its typical **9 V / 620 ns forward overshoot at 1.5 A, 12 A/us, 25 C** does not prove <=5 V at the LED. Ordinary RUN/TEST/switching and nearby-lightning response remain unqualified. Installation-polarity LED damage is accepted with spares, not claimed immunity. See [ELECTRICAL section 9](pcb/ELECTRICAL.md).
- **C5, powered faults:** The ordered 2 A Littelfuse 0217002.MXP fuse has published DC interrupting data, but cable-limited faults and PSU hiccup behavior may prevent prompt clearing. GDT powered recovery remains unresolved, **not an observed latched failure**. The [selected fault screens](pcb/DESIGN_BOUNDS.md#fault-screen) include sustained path heating at source currents below the fuse rating; these conditional load lines are neither extinction predictions nor worst-case fault bounds. D3/D4 do not change the upstream GDT paths.
...
repetitive pulses. Continuous-safe normal TEST remains mandatory. Energized
cattle fencing is prohibited near the **whole 4 km boundary cable, every station
and the hub**; the former close-parallel exposure case is withdrawn, not newly
qualified or evidence of zero induction/lightning risk.

## 4. Hardware Component Selection & Specifications

The authorized design has **16 electrical parts, all top-side: eight SMT and eight THT**, plus four mechanical hole footprints. These are design counts, not inspected hardware or a native-check result. Codes identify sourcing, not reserved stock or verified footprint compatibility.

| References | Manufacturer / MPN | Sourcing code | Component data and design role |
| :--- | :--- | :--- | :--- |
| D1-D4 | **Vishay General Semiconductor BYG23T-M3/TR** | C145454 | 1300 V repetitive reverse, 1 A, SMA. D1/D2 series; D3/D4 negative output shunts. Recovery and voltage ratings do not qualify the LED transient response. |
| R1, R2 | **TE Connectivity 35212K2FT** | **[C4129105](https://jlcpcb.com/partdetail/C4129105)** | CGS 3521 Series, 2512 SMT, 2.2 kohm, 2 W P70 under the cited mounting basis, 1%, +/-100 ppm/C, AEC-Q200 qualified. Prototype BOM only. Yageo SR2512FK-7W2K2L / C876850 is ordered for future production, subject to separate review. No resistor insertion holes or lead forming. Retained alternate-land, solder-process and OneGel thermal acceptance remain open. |
...
The old onsemi 1N4007G, Vishay MBE04140C2201FC100 and axial **PR02000202201FA100** are **retired from the design**, not default substitutions; procurement history remains in MATERIALS. The prior **HP122WF2201T4E / External** selection is **superseded, not ordered**. **C2791283 is HP122WJ0472T4E, 4.7 kohm / 5%, not that prior 2.2 kohm / 1% HP122WF2201T4E selection.** Its wrong mapping in commit `02661d8` remains rejected. Current 35212K2FT metadata uses **manufacturer `TE Connectivity`, C4129105, `Sourcing=LCSC` and an empty `Sourcing Reference`**, resolving resistor sourcing for the prototype order with verified stock at JLCPCB. The generic rule remains: unresolved `External` rows require an empty code and exact matching HTTPS reference and block **both prototype and production publication**. Sourcing resolution is not PCBA-job allocation; whole-BOM `allocation_verified` remains `false`. No agent upload or order is authorized.
...
| Diodes | D1/D2 origins (130.80,104.50)/(130.80,140.50), 0 degrees, cathodes at X=132.90 and anodes at X=128.70. D3/D4 remain at (135.00,99.00)/(135.00,146.00), 180 degrees, cathodes at X=132.90 and anodes at X=137.10. Straight cathode links at X=132.90 are 1.80 mm wide and 5.50 mm long; shunt anode-to-B routes to J_LED_A/C pin 2 are 1.80 mm wide. All use `DogFence:BYG23T_SMA_K_Right`; check physical polarity after rotation. |
| Resistors | R1/R2 origins (120.00,104.50)/(120.00,140.50), 0 degrees, pad centres X=116.875/123.125. Retained **project lands**, not TE's recommended pattern: 1.35 X x 3.70 Y mm rectangles at local X=+/-3.125, 4.90 mm inner gap / 6.25 mm pitch. TE's maximum body 6.45 x 3.40 x 0.65 mm fits the existing body envelope; that is not exact land-pattern or solder-fit approval. The 8.10 x 4.20 mm courtyard and zero additional mask/paste margins remain. No resistor holes, axial standoff or forming; no folded takeoff or additional via. |
| Other orientation | J_IN opens left (0 degrees), J_EARTH right (180 degrees); J_IN A/B/C run north to south. Footprint origins/rotations are not automatically assembler centroids/model rotations. |
...
TE recommends **1.50 X x 3.00 Y mm lands, 5.00 mm inner gap and 6.50 mm pitch**, unlike the retained resistor pattern above. [ASSEMBLY](pcb/ASSEMBLY.md) controls the alternate project lands and their existing supplier placement/stencil/solder-acceptance hold. Body-envelope compatibility is not a "100% fit" claim or authorization to change the layout.
...
The retired 1N4007G's 0.90-to-1.10 mm hole correction is historical. **D1-D4 now use SMA lands, not insertion holes**: each land is **2.50 x 2.00 mm**, at local X=+/-2.10 mm, with a **1.70 mm inner gap** and zero additional mask/paste margins. Local pad 1/cathode is +X; D3/D4's 180-degree placement puts it left globally. Actual stencil, solder and placement acceptance remain required.

The selected hole/pad schedule is **KF128 2.00/3.20, KF129 2.00/2.80, B5G470L 1.40/2.80, earth GDTs 1.50/3.00 mm**. Minimum nominal ring is 0.40 mm; earth GDT maximum lead is 1.05 mm. Other controlled pin envelopes and the separate hole-position/formed-pattern budget remain in [fit evidence](pcb/ASSEMBLY.md#hole-fit-evidence). Actual allocated THT parts still must pass free-insertion, GDT forming/standoff and solder-process inspection; supplier CAD or nominal pitch is not that acceptance. 2512 SMT resistor acceptance concerns its SMT lands, body, placement and solder process, not insertion or axial forming.
...
`make check` performs file verification and refreshes the generated CPL reference only after all file/artifact checks pass, **without publication**. `make gerbers` / `make prototype` run the same complete checks and may publish **FILE-VERIFIED PROTOTYPE ONLY** packages with all five ledger holds explicitly deferred, not closed. **The selected TE 35212K2FT / C4129105 resolves resistor sourcing for the prototype order, not any file or qualification gate**; unresolved external sourcing still blocks both modes. Use [REMEDIATION](REMEDIATION.md) for the latest revision-specific results.

`make all`, `package`, `release`, `production`, `drills`, `ipc`, `bom`, `cpl`, `zip-gerbers` and `zip-flytest` retain the production gates, including C4/C5/W3/W1/W4. Production wins if mixed with prototype goals, regardless of goal order. Reports are in `build/reports/` and `build/drc_report.*` / `build/erc_report.*`; `build/status.json` identifies the attempt. Require a matching `build/manifest.json` with validated revision/source/artifact hashes and the correct pair: **`status=prototype`, `mode=prototype`**, or production **`status=verified`, `mode=build`**. Filenames or `status=verified` alone are insufficient. Neither mode authorizes upload, an order or field use. Run `make clean` separately and preserve desired evidence first.
...
| `build/README.txt` | Generated mode/hold notice, also included inside both `Gerbers.zip` and `FlyTest.zip`. The same public filenames serve both modes; this notice travels with the files but does not replace the matching manifest. |
...
| :--- | :--- |
| **P: File-verified prototype artifacts** | Same strict file/artifact and sourcing checks, matching prototype manifest, and all open ledger holds recorded as deferred. Authorized artifact scope only; supplier CAM/placement/allocated parts and separate user authorization are still needed before an actual order. |
| A: Production file ready | Original engineering/file-release review: correct source/circuit/library/rule data, passing checks, reviewed artifacts, synchronized documentation and evidence-backed hold closure. Not closed by P. |
| B: Original prototype-order gate | Original Gate A plus required part-fit, via/solder/GDT-forming process, CAM and placement acceptance, and a controlled test scope. Not closed by prototype file generation. |
| C: Field release | Required normal/protection/fault, continuous thermal, 5 m daylight, full-hub cut, RF/site and environmental evidence, accepted earthing, maintained cattle-fence prohibition and documented performance limits. Installation-polarity LED immunity is excluded, not passed. |

**P is separate from the original A/B/C production/field holds.** Do not require switch/environmental physical qualification or enclosure dry-fit before boards exist to generate prototype artifacts. GDT fit/forming, panel/process, exact MPN/allocation and supplier CAM/placement remain explicit order tasks; PR02 procurement/forming is retired. Prototype-dependent tests remain deferred, not passed. No upload or order is authorized by the present file-generation scope, and neither documentation nor five basic bench passes closes C4/C5 or permits full rollout.
