...
This is a risk register and qualification boundary, not a safety certificate or measured reliability prediction. [REMEDIATION.md](REMEDIATION.md) controls settled requirements, the live issue checklist and [release gates A/B/C](REMEDIATION.md#release-gates). [pcb/ELECTRICAL.md](pcb/ELECTRICAL.md) records the three-core DC analysis and candidate-part research. **P3 changes leave the circuit/BOM at 14 parts; no new LED reversal/pulse protection or coordinated source/GDT shutdown circuit is selected or fitted. C4 and C5 require real implementation and test evidence, not corrected prose.**
...
**User-accepted risk, 2026-09-07:** indicator damage from accidental low-voltage installation polarity errors is accepted, with spare LEDs for replacement. Reversed flying-lead survival is no longer a required feature. The failure mechanisms above remain true; accepting replacement is not evidence of immunity or permission to continue powering suspect wiring.

**C4 remains open for the simplified protection and ordinary-transient scope.** A single antiparallel diode per PCB connector can reduce negative connector voltage, but does not cover reversed flying leads or regulate forward pulse current. The 16-part candidate is not fitted. Assess actual RUN/TEST, switching and repetitive cattle-fence exposure and the claimed protection response; do not impose the historical 40-part circuit merely because an unspecified surge cannot be guaranteed harmless. Recheck clean-cut observability after any branch change.
...
Same-end TEST has distributed loading and a source-to-far-end current gradient. The [P2 continuation](pcb/ELECTRICAL.md#8-continuation-decision-2026-09-07) and [conditional design bounds](pcb/DESIGN_BOUNDS.md) analyze the **41-station** network with a proposed **35-37 V TEST-feed window, nominal 36 V**. **Neither feed limit is enforced in hardware**; include actual adjustment, hub/interface losses, ripple and start-up rather than treating PSU OVP as a 37 V limiter. Final cable/contact, LED and protection loading still need supported limits. Include credible negligible-cable staging, not just five boards.

The **4.6681 mA nonuniform far-end corner at 35 V** uses one high-drop/Rmax branch with the other branches at zero drop/Rmin. It is not an exhaustive joint-tolerance minimum, an optical result or a guaranteed **6 mA floor**. Uniform-load results can overstate the weakest LED's current; a new clamp's loading is not covered by the unchanged-branch study.

With the baseline 2.2 kohm resistors, the proposed **37 V zero-drop/Rmin screen gives 0.631876 W per resistor** and a **71.534 degrees C rating-only local ambient ceiling**. Neither is a measured temperature or safe continuous qualification. The Vishay MBE0414 **1 W power-mode rating differs from its 0.65 W standard-mode rating** and both need their specified film-temperature, derating and mounting conditions. The **40.39597 V accessible-adjustment screen still gives 0.753190 W**, above standard-mode P70, until a real interface enforces a lower ceiling. Internal heating and direct sun reduce margin; a higher-wattage resistor dissipating the same power does not automatically reduce enclosure heat or fit the footprint.
...
More ordinary candidates are **repetitive coupling from the parallel cattle
wire**, switching/interruption of cable current, contact bounce or loose/arcing
connections, and source startup/disturbances. These are mechanisms to assess,
not a claim that normal operation currently produces destructive pulses.
Accidental direct contact with an energized cattle wire is a separate wiring/
mechanical fault, not the same case as 0.5 m inductive/capacitive coupling.
The cattle energizer's kV output is not automatically the voltage induced in
DogFence. Ordinary switching/RUN/cattle exposure has not been waived as an
acceptable cause of repeated failures.
...
The [adopted P3 limits](pcb/ASSEMBLY.md) and [DFM evidence](pcb/DFM_EVIDENCE.md) now control all maximum-body boxes and enlarged component holes with independent pin-pattern budgets. **E limits are procurement envelopes, not guaranteed or measured lots**; W4 actual pin/pattern/body fit and solder/forming acceptance remain open. The metric Ruilon SMT pattern is selected, not awaiting figure interpretation, but **W1 retains the manufacturer's metric/inch contradiction and actual solder/stencil/CAM acceptance**. These distinctions do not weaken the geometric guardrails or close release gates.
