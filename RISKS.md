...
The updated PR02 model provides concrete limits on overcurrent-only protection: in its declared upper-source/cable screen, a conditional far-end 10 V AB arc carries **0.127800 A / 1.279635 W**, while total source demand is **0.984994 A**. A near-source 100-ohm fault dissipates **16.050680 W at only 1.330681 A total**. These are selected prospective load lines, not maximum fault bounds or observed latched failures. They do not establish physical extinction or non-extinction; actual holdover/source-recovery evidence remains necessary. D3/D4 do not alter the upstream GDT follow-current paths.

### Normal Duty

Same-end TEST has distributed loading and a source-to-far-end current gradient. The [current electrical record](pcb/ELECTRICAL.md) and [conditional design bounds](pcb/DESIGN_BOUNDS.md) analyze the **41-station PR02/BYG23T network** at nominal 36 V, a declared **35 V feed floor and 40.39597 V upper screen**. These are not enforced source limits or guaranteed combined ratings. The former **35-37 V window was only a proposal**; include actual adjustment, hub/interface losses, ripple and startup rather than treating PSU OVP as a 37 V limiter. Qualifying the real normal source range is an alternative to imposing an active cutoff. Include credible negligible-cable staging, not just five boards.

The new **4.090396 mA nonuniform far-end LED corner at 35 V** uses a **5.2 V high-drop/Rmax branch**, other branches at zero drop/PR02 Rmin, and **50 uA conditional clamp diversion**. It is not an exhaustive joint-tolerance minimum, measurement or guaranteed optical floor. Uniform loads can overstate the weakest LED's current; the separate loose all-station comparison gives only **0.195041 mA after diversion** within the declared envelope, not predicted brightness. Clamp leakage diverts branch current rather than adding it again to source demand. Series-diode reverse leakage and OFF-state glow are not solved by the ideal one-way cut model.

The selected **Vishay BCcomponents PR02000202201FA100** is copper-lead **2.2 kohm, 2 W P70, 1%, +/-250 ppm/K**, not the retired MBE0414 or 1.3 W FeCu PR02. With the selected resistance-temperature screen through 125 C, **40.39597 V / zero drops / Rmin = 2120.8275 ohms** gives **19.047268 mA and 0.769432871 W per resistor**, or **1.561875985 A / 63.0935 W for all 82 branches with zero cable resistance**. PR02 derates to zero at 155 C ambient; its specific hot-spot maximum is 220 C. The rating-only ambient ceiling for this load is **122.299 C**, not a predicted or acceptable assembly temperature, and the screen exceeds its **0.705882 W allowance at 125 C ambient**. Resistor temperature in the resistance calculation is an input, not a self-heating result.

The **>=1.00 mm PR02 body-to-PCB standoff** is a mounting requirement, not actual process acceptance. Vishay's typical 75 K/W mounted example would imply about **57.7 K hot-spot rise** at 0.769433 W, not OneGel thermal resistance or a potted prediction. Nominal heat remains about **0.50 W per resistor**. No closed/gel-filled thermal test has been performed; the new rating does not establish cool operation, material compatibility or long life.
...
The [controlled assembly limits](pcb/ASSEMBLY.md) and [DFM evidence](pcb/DFM_EVIDENCE.md) distinguish maximum-body boxes and component holes from actual fit. PR02's **0.83 mm maximum copper lead, retained 15.24 mm pitch and 1.40/2.40 mm holes/pads** replace the old MBE identity; D1-D4 now use **2.50 x 2.00 mm SMA lands at +/-2.10 mm, 1.70 mm inner gap**, not diode insertion holes. **E limits remain procurement envelopes, not guaranteed or measured lots**; W4 actual pin/pattern/body fit and solder/forming acceptance remain open. The metric Ruilon SMT pattern is selected, but **W1 retains the manufacturer's metric/inch contradiction and actual solder/stencil/CAM acceptance**, including the new diode lands' process review. All protected rails/vias/earth/LED-terminal geometry remains; no folded routing or extra via is implemented. These file choices do not close release gates.
...
In particular, **C4 LED transient qualification**, **C5 source/GDT coordination**, **W3 actual continuous thermal behavior** and **W1/W4 process/fit acceptance** remain open after implementation. PR02 external sourcing is also unallocated, with no verified LCSC code; metadata parity is not JLC procurement approval. Keep outstanding switch DC/contact/transfer evidence, ordered-PSU receipt/settings, cable manufacturing-revision/resistance/exposure applicability, insulation, OneGel process/material and Essentra acceptance, enclosure fit, thermal/visibility/RF/site tests and installer earthing review explicit. Retain one unpotted reference among the five prototypes and obtain approval for additional samples when the existing qualification scope requires them.
