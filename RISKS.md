...
This is a risk register and qualification boundary, not a safety certificate or measured reliability prediction. [REMEDIATION.md](REMEDIATION.md) controls settled requirements, the live issue checklist and [release gates](REMEDIATION.md#release-gates). [pcb/ELECTRICAL.md](pcb/ELECTRICAL.md) records the three-core DC analysis and protection evidence. The authorized **16-part 2512 SMT/BYG23T hybrid** uses selected **TE Connectivity 35212K2FT / [C4129105](https://jlcpcb.com/partdetail/C4129105)** (2.2 kohm / 2 W / 1%, AEC-Q200, in stock at JLCPCB) for R1/R2 in the prototype BOM only, two series SMA diodes and two negative-voltage output shunts, with the existing GDTs and terminals. The user has also ordered **Yageo SR2512FK-7W2K2L / [C876850](https://jlcpcb.com/partdetail/C876850)** with an advised 20-day wait for future production, not confirmed receipt or allocation. The prior Uni-Royal PS12 order was cancelled/refunded due to stockout; MATERIALS preserves its historical 110-part order. Identity mapping is resolved, not PCBA-job allocation. This is not assembled or qualified hardware. **C4, C5, W3, W1 and W4 remain open**; no powered-GDT shutdown circuit or physical thermal/transient result is established.
...
Same-end TEST has distributed loading and a source-to-far-end current gradient. The [current electrical record](pcb/ELECTRICAL.md) and [conditional design bounds](pcb/DESIGN_BOUNDS.md) analyze the **41-station TE-prototype / BYG23T network** at nominal 36 V, a declared **35 V feed floor and 40.39597 V upper screen**. These are not enforced source limits or guaranteed combined ratings. The former **35-37 V window was only a proposal**; include actual adjustment, hub/interface losses, ripple and startup rather than treating PSU OVP as a 37 V limiter. Qualifying the real normal source range is an alternative to imposing an active cutoff. Include credible negligible-cable staging, not just five boards.

Use [DESIGN_BOUNDS](pcb/DESIGN_BOUNDS.md#normal-current) for the prototype resistor's nonuniform current corners and loose comparison floor, not retired PS12/PR02 evidence or unreviewed Yageo extrapolations. A **5.2 V high-drop/Rmax branch**, other branches at zero drop/Rmin, and **50 uA conditional clamp diversion** are screening assumptions, not an exhaustive joint-tolerance minimum, measurement or guaranteed optical floor. Uniform loads can overstate the weakest LED's current. Clamp leakage diverts branch current rather than adding it again to source demand. Series-diode reverse leakage and OFF-state glow are not solved by the ideal one-way cut model.

The prototype **TE Connectivity 35212K2FT** is **CGS 3521 / 2512 SMT, 2.2 kohm, 1%, +/-100 ppm/C**. [TE 9-1773463-5, Rev G, 02/2025](https://www.te.com/commerce/DocumentDelivery/DDEController?Action=showdoc&DocId=Data+Sheet%7F9-1773463-5%7FG1%7Fpdf%7FEnglish%7FENG_DS_9-1773463-5_G1.pdf) supports that TCR magnitude at 2.2 kohm and 2 W at 70 C ambient, derating to zero at 155 C ambient; -55..155 C is its operating range, not a verified TCR test range. The retained **25 C resistance reference and -30..125 C linear screen are engineering assumptions**; neither a 25 C reference nor -55/+125 C TCR test endpoints are verified TE facts. [Thermal screens](pcb/DESIGN_BOUNDS.md#thermal-screens) retain the same conditional numerical power results. Resistor temperature is an input, not a self-heating result or qualified operating point.

TE page 3's mounting/thermal reference is a **four-layer PCB with 2 oz outer / 4 oz inner copper**; this project has **two layers, 2 oz each**. Applying P70/derating to this mounting requires evidence and measured chip/termination/pad/PCB and material-interface temperatures. The difference does not prove two-layer failure, but the catalogue curve alone cannot close W3. The 155 C zero-power endpoint is not an acceptable powered-chip or gel temperature.
...
**TE prototype results do not establish Yageo thermal qualification.** Before production release, separately review the ordered **SR2512FK-7W2K2L / C876850** manufacturer's rating, land-pattern, thermal, pulse and process evidence, update the controlled BOM/CAD, libraries, models and notes and reverify. A matching resistance/wattage or an order with a lead time is not interchangeable qualification or an implemented production BOM.
...
| Pulse coordination | A rectifier's repetitive reverse rating or single forward-surge rating and a resistor's steady wattage do not establish arbitrary pulse survival or protect a 5 V-reverse LED. TE's **250 V working / 500 V overload family ceilings** do not override the **66.33 V catalogue working-voltage arithmetic at 2 W / 2.2 kohm**, itself conditional on mounting/derating. The cited TE sheet does not establish the historical PS12 `2.5 * RCWV` / 5 s overload test or PS one-pulse power/voltage curves. Do not transfer those data to TE or use TE results to qualify Yageo. Verify exact-part voltage, current, pulse energy, timing, repetition and powered recovery for the actual network under the controlled electrical notes. |
...
The [controlled assembly limits](pcb/ASSEMBLY.md) and [DFM evidence](pcb/DFM_EVIDENCE.md) distinguish maximum-body boxes, SMT lands and retained component holes from actual fit. TE's **6.45 x 3.40 x 0.65 mm maximum body** fits the existing body envelope, but the retained **1.35 x 3.70 mm rectangular lands at local X=+/-3.125** are **alternate project lands, not TE's recommended pattern**. Supplier placement/stencil/solder acceptance remains required; a body match is not a manufacturer-exact or 100% fit claim. The **8.10 x 4.20 mm courtyard** remains; D1-D4 use **2.50 x 2.00 mm SMA lands at +/-2.10 mm, 1.70 mm inner gap**. Both retain zero additional mask/paste margins; no resistor/diode insertion holes or axial resistor forming/standoff remain. Retained Kefa/GDT **E limits are procurement envelopes, not guaranteed or measured lots**; W4 actual pin/pattern/body fit and solder/GDT-forming acceptance remain open. **W1 retains the Ruilon metric/inch contradiction and actual solder/stencil/CAM acceptance**, alongside TE/BYG23T process review. All protected rails/vias/earth/mounts/LED-terminal geometry remains; no folded routing or extra via is implemented. These file choices do not close production/field holds.
...
In particular, **C4 LED transient qualification**, **C5 source/GDT coordination**, **W3 actual continuous thermal behavior** and **W1/W4 process/fit acceptance** remain open. **TE Connectivity 35212K2FT / C4129105 resolves the prototype resistor procurement blocker (in stock at JLCPCB); Yageo SR2512FK-7W2K2L / C876850 is ordered with a 20-day wait for separately reviewed future production; the prior Uni-Royal PS122WF2201T4E order was cancelled/refunded due to stockout. HP122WF2201T4E sourcing is superseded, not ordered; C2791283 remains the rejected 4.7 kohm / 5% HP122WJ0472T4E.** Unresolved external rows still block both publication modes. Neither stock nor an order establishes PCBA-job allocation; whole-BOM `allocation_verified` remains `false`. Latest file/publication results are in REMEDIATION, not implied by these documentation corrections. Keep outstanding switch, source, cable, OneGel, enclosure, thermal/visibility/RF/site and earthing qualification explicit without restarting that research for prototype files. Retain an unpotted reference and obtain approval for additional samples when the existing qualification scope requires them.
