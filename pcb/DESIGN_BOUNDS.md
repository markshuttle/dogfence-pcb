...
**16-part PR02 / BYG23T circuit**. This work owns only `scripts/analyze_limits.py`,
`scripts/design_bounds.py`, their two focused test files and this note. The
nodal solver is retained; source/layout/BOM guards, shared ELECTRICAL record,
manufacturing/build and approval integration belong to the parent/other owner.
These are calculations, not hardware qualification or closure of C4/C5/W3.

## Selected Circuit

```text
WIRE_A -> R1 PR02000202201FA100 -> D1 BYG23T-M3/TR -> LED_A_POS -> external LED -> WIRE_B
WIRE_C -> R2 PR02000202201FA100 -> D2 BYG23T-M3/TR -> LED_C_POS -> external LED -> WIRE_B
...
is added. Main-record synchronization belongs to the parent.
...
- **R1/R2:** Vishay **PR02000202201FA100**, copper-lead PR02, 2200 ohm,
  +/-1%, **+/-250 ppm/K**, **2 W P70**, not the 1.3 W FeCu version [7]. The
  selected -30..125 C resistance screen gives **2120.8275..2280.3275 ohm**:
  `Rmin = 2200 * 0.99 * (1 - 250e-6 * 105)`. Temperature is an input, not a
  self-heating result or a 125 C PR02 film limit. The helper permits explicit
  TCR screens within the datasheet's -55..155 C test range; it does not
  automatically extrapolate to the 220 C hot spot. Aging, assembly/solder drift
  and unbudgeted tap/harness losses are excluded and can consume current margin.
...
Sources [1]-[6] preserve the earlier evidence. Only the two requested primary
Vishay documents [7]/[8] were freshly fetched for this update on 2026-09-07.
Direct PDF webfetch returned binary; text was read through `https://r.jina.ai/`
prefixed to those exact manufacturer URLs. This is an extraction aid, not an
archived original, unreadable-figure approval or vendor reply. No new broad
research, pulse-curve qualification or supplier acceptance is claimed.
...
7. [Vishay PR01/PR02/PR03, 28729, 08-Jul-2025](https://www.vishay.com/docs/28729/pr010203.pdf): pp. 1-3 Cu/FeCu ratings, ordering and generic film entry; pp. 5/7/8 ambient derating, mounting/hot-spot examples and PR02-specific 220 C limit; pp. 14-16 TCR/test conditions and dimensions.
...
| Uniform 5.2 V high Vf / PR02 Rmax at every branch | 35 | 5.686756 | 5.636756 | 0.659622 |
| Only A40 high Vf; all other Vf zero; Rmax everywhere | 35 | 4.451650 | 4.401650 | 0.773731 |
| Only A40 high Vf / Rmax; all other Vf zero / Rmin | 35 | 4.140396 | 4.090396 | 0.811361 |
| Previous case with the other positive rail C at zero ohms | 35 | 3.656169 | 3.606169 | 0.870078 |
| Zero cable/contact R, zero Vf / PR02 Rmin everywhere | 40.39597 | 19.047268 | 18.997268 | 1.561876 |
...
Uniform Vf overstates A40 current by **1.235106 mA** even before the additional
**0.311253 mA** reduction from asymmetric resistor tolerance. Lower resistance
...
leakage bound. The positive-rail lower value is **24.7282110028 V**, B upper is
**18.9694383635 V**, and this intentionally loose all-station floor becomes
**0.2450405213 mA branch / 0.1950405213 mA LED after diversion**. It is much
...
named historical regression, not silently reused for PR02/BYG23T. Even merely
...
The independent upper bound is **19.0472681064 mA/branch**,
**1.5618759847 A / 63.0934954228 W** for 82 branches, and
**38.0945362129 mA at the local PCB B tap**. WAGO through-splices
carry distributed perimeter current, not just that tap current. One LRS-75-36
needs at least **83.4570%** of its rated current/power capacity available for this
...
PR02 with copper leads is **2 W at local ambient <=70 C**, derating linearly
to **zero at 155 C ambient** [7]:

`P_allow(T_local) = 2 * clamp((155 - T_local)/(155 - 70), 0, 1)`.

**155 C is not the film limit.** Page 8 specifies the PR02 hot-spot maximum
as **220 C**, despite page 2's generic **250 C film** entry. Use the PR02-specific
limit in this analysis and do not infer permission for gel, solder, cable or
nearby material to reach 220 C. Power is conditional on actual film/hot-spot
temperature and mounting heat flow; ambient derating alone is not W3 closure.
The former MBE 0.65 W standard / 1 W power-mode helper is no longer the selected
resistor model.
...
| 36 V, PR02 Rmin / zero Vf | 16.974506 | 0.611082 | 1.222164 | 129.029 |
| Legacy 37 V proposal, now PR02 Rmin / zero Vf | 17.446020 | 0.645503 | 1.291006 | 127.566 |
| Current 40.39597 V screen, PR02 Rmin / zero Vf | 19.047268 | 0.769433 | 1.538866 | 122.299 |

At the current upper screen, **0.7694328710 W/resistor** leaves
**1.2305671290 W below PR02 P70**. The rating-only local ambient ceiling is
**122.2991029821 C**. At 125 C ambient, allowance is only **0.7058823529 W**,
so this screen exceeds it by **0.0635505181 W**. Do not extrapolate below
...
**65.127989 V** is **not** an allowed source voltage: it exceeds the cable's
...
R/TCR screen, 20 mA with zero drops corresponds to **42.41655 V**, not a hardware
...
That heat is not removed by the 2 W resistor rating, and clamp leakage is not
added again to the PSU current or whole-branch electrical-input budget.
...
heat bounds it by **1.5388657420 W** at 40.39597 V. This already includes any
...
**19.4948780656 K/W**. At 60 C local ambient / 110 C film, the paired-load effective
film-to-local allowance is **64.9829268854 K/W per resistor**. These are required
...
Vishay's **typical 75 K/W mounted PR02 example**, with **>=1 mm body-to-PCB
standoff** in the mounting examples, is **not OneGel thermal resistance**. At
0.7694328710 W it implies **57.7074653257 K hot-spot rise**, or about 117.707 C
from 60 C ambient in that example, above the proposed 110 C target. This shows
why rating margin does not prove a cool body; it neither predicts OneGel
temperature nor proves this mounting fails a real potted test. Establish actual
body/lead/estimated-film and adjacent-material temperatures with both branches
powered to steady state, including solar exposure and output-short conditions.

Named historical regressions retain the former MBE **2166.5655 ohm** minimum,
**0.631876 W at 37 V** and **0.753190 W at 40.39597 V**, rather than calling them
PR02 limits. They are not prior physical qualification.
...
slope**. At 40.39597 V / declared cable / zero-Vf PR02 Rmin loads, a near-source
100 ohm fault dissipates **16.0506799262 W** at **0.4006329982 A in the fault**
with just **1.3306807123 A total**, below even the healthy upper envelope.
...
The named new far-end 10 V arc case gives **0.1278001239 A / 1.2796345266 W**
in the path and **0.9849936902 A total**. Neither it nor the higher old nominal
arc value below is a maximum over cable/load/fault conditions or evidence of
...
On **Python 3.14.4**, the final focused runs passed **23 electrical tests and
13 design-bound tests**. Both human CLIs exited 0; JSON and error paths passed
in the tests, and the design-bounds JSON CLI also exited 0 directly.
`git diff --check -- scripts/analyze_limits.py scripts/design_bounds.py tests/test_electrical.py tests/test_design_bounds.py pcb/DESIGN_BOUNDS.md`
passed. These results concern only the five owned analysis files.

The suites cover selected PR02 rating/TCR, historical named values, BYG23T
...
and ELECTRICAL/main-record updates after source edits settle.
