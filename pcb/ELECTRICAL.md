...
```text
WIRE_A -> R1 35212K2FT 2.2k 2W -> D1 A(2), K(1) -> LED_A_POS -> J_LED_A.1
WIRE_C -> R2 35212K2FT 2.2k 2W -> D2 A(2), K(1) -> LED_C_POS -> J_LED_C.1
...
Defaults are **41 stations**, numbered 0 through 40 at 0.1 km spacing; 82 branches; 36 V; and the historical illustrative 6.9 ohm/km per core. The 2.1 V LED / 0.7 V series-diode defaults preserve that comparison, **not characterized APEM/BYG23T I/V data**. `design_bounds.py` uses TE 35212K2FT tolerance/TCR with a **declared 25 C resistance reference and linear temperature screen**, the unchanged conditional cable/contact ceiling and a **5.2 V high-drop screen** (3.3 LED + the BYG23T 1.9 V maximum tested at 1 A/25 C). Neither junction test point is a full-temperature indicator-current guarantee. `R_nom_25` below is the assumed model reference, not a TE test-condition claim.
...
**Historical PS12 synchronization, 2026-09-08:** all four reproduction commands above passed
...
unchanged; tests then checked PS12 identity, 500/1000/500 V family ratings and the
...
New/updated assertions distinguish assumed temperature conditions, unknown TE
test parameters and unverified catalogue-rating transfer. No native/build/
workflow/approval operation, commit, upload, order or physical test was run.
...
investigations**, not the selected TE 3521/BYG23T limits. Its original requirement
...
| S3 | [Vishay 28766, MBx/SMA](https://www.vishay.com/docs/28766/mbxsma.pdf), 11-Jul-2018; [text extraction](https://r.jina.ai/https://www.vishay.com/docs/28766/mbxsma.pdf) | **Historical, retired MBE.** Pages 1-4/8-10: rating modes, film limits, TCR/tolerance, derating and separate pulse constraints, not TE 3521 data. |
...
screen**, not the current TE prototype calculation. `scripts/design_bounds.py` still
